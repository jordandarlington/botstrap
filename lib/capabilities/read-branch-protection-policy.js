...
const readBranchProtectionPolicyCapability = {
    key: "read-branch-protection-policy",
    description: "Reads GitHub branch protection policies for one or more repository branches.",
    runtimes: ["github", "cli"],
    ...
    async handle(context, config = {}) {
        ...
        if (!owner || !repo || branches.length === 0) {
            throw new Error("read-branch-protection-policy requires owner, repo, and at least one branch");
        }

        const results = [];

        for (const branch of branches) {
            const policy = await readBranchProtectionPolicy(context, {
                owner,
                repo,
                branch,
                token: config.token,
            ...
            }, "read-branch-protection-policy: branch protection status");
...
async function readBranchProtectionPolicy(context, params) {
    ...
    return readBranchProtectionPolicyViaFetch(params);
}

async function readBranchProtectionPolicyViaFetch({ owner, repo, branch, token }) {
