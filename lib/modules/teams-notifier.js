const teamsNotifierModule = {
    key: "teams-notifier",
    events: ["pull_request.opened", "pull_request.reopened", "issue_comment.created" ],

    async handle(context, config = {}, eventName) {
        ...
        if (eventName === "issue_comment.created") {
            ...
            const title = config.title || `New comment on ${itemType} #${issue.number}`;
            const text = config.message || [
                `${sender.login} commented on ${itemType} #${issue.number} in ${repository.full_name}.`,
                "",
                ...
                sections: [
                    {
                        facts: [
                            ...
                            { name: isPr ? "Pull Request" : "Issue", value: `#${issue.number}` },
            ...
            context.log.info({ payload, repository: repository.full_name }, "teams-notifier: posting comment webhook");

            await postToTeams(webhookUrl, payload, context, repository.full_name);
            return;
        }

        if (["pull_request.opened", "pull_request.reopened"].includes(eventName)) {
            const webhookUrl = config.webhookUrl || process.env.TEAMS_WEBHOOK_URL;
...
async function postToTeams(webhookUrl, payload, context, repositoryName) {
    context.log.info(
        { webhookUrlConfigured: true, repository: repositoryName },
        "teams-notifier: posting webhook",
    );
    ...
    context.log.info(
        { status: response.status, repository: repositoryName },
