...
function getCapabilityConfig(moduleConfig, capability) {
    ...
    return {
        ...
        ...moduleConfig,
        ...capabilityConfig,
        ...moduleConfig.runtimeOverrides,
    };
}

async function runCapabilities(context, moduleConfig, moduleWithCapabilities, eventName, options = {}) {
    const results = [];
    ...
    for (const rawCapability of moduleWithCapabilities.capabilities) {
        ...
        try {
            ...
            if (!supportsRuntime(capabilityWithHandle, options.runtime)) {
                continue;
            }
...
async function runBots(context, configs, eventName, options = {}) {
    try {
        ...
        for (const configName of configs) {
            ...
            try {
                ...
                if (!supportsRuntime(moduleDefinition, options.runtime)) {
                    continue;
                }

                if (eventName && moduleDefinition.events && !moduleDefinition.events.includes(eventName)) {
                    continue;
                }
